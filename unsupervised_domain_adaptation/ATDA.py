...
class ATDA(object):
	...
	opt = optimizers.SGD(lr=lr, momentum=0.9)
        modelF1F2.compile(loss=['categorical_crossentropy', 'categorical_crossentropy'], optimizer=opt, metrics=['accuracy'])
        modelF1F2.fit([X_source_train], [y_source_train, y_source_train], epochs=n_epoch, batch_size=batch_size_F1F2, verbose=2, validation_split=0.2)

        modelFt = Model([inp], [Ft])
	opt = optimizers.SGD(lr=lr, momentum=0.9)
        modelFt.compile(loss=['categorical_crossentropy'], optimizer=opt, metrics=['accuracy'])
        modelFt.fit([X_source_train], [y_source_train], epochs=n_epoch, batch_size=batch_size_Ft, verbose=2, validation_split=0.2)

        return modelF1F2, modelFt


    def second_training(self, inp, X_source_train, y_source_train, X_target_train, y_target_pseudo_labels, F1, F2, Ft, n_epoch, batch_size_F1F2, batch_size_Ft, lr, X_target_valid, y_target_valid):
        ...
        F1F2.fit([data], [y, y], epochs=n_epoch, batch_size=batch_size_F1F2, verbose=2, validation_split=0.2)

        Ft_model = Model([inp], [Ft])
	opt = optimizers.SGD(lr=lr, momentum=0.9)
        Ft_model.compile(loss=['categorical_crossentropy'], optimizer=opt, metrics=['accuracy'])
        Ft_model.fit([X_target_train], [y_target_pseudo_labels], epochs=n_epoch, batch_size=batch_size_Ft, verbose=2, validation_data=(X_target_valid, y_target_valid))
        return F1F2, Ft_model

    def iterate_algorithm(self, inp, F1, F2, Ft, X_source_train, y_source_train, X_target_train, y_target_train, n_epoch, k, batch_size_F1F2, batch_size_Ft, threshold, lr,X_target_test,y_target_test, \
	...
	for train_index, test_index in sss.split(X_target_train, y_target_train):
	    X_target_train_, X_target_valid = X_target_train[train_index, :], X_target_train[test_index,:]
	    y_target_train_, y_target_valid = y_target_train[train_index,:], y_target_train[test_index,:]

	print X_target_valid.shape, y_target_valid.shape, X_target_test.shape,y_target_test.shape, X_source_train.shape, y_source_train.shape, X_target_train_.shape, y_target_train_.shape

        Ft = self.iterate_algorithm(inp, F1, F2, Ft, X_source_train, y_source_train, X_target_train_, y_target_train_, n_epoch, k, batch_size_F1F2, batch_size_Ft, threshold, lr, X_target_test,y_target_test, \
			X_target_valid, y_target_valid)
